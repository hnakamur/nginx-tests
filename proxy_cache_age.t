...
my $t = Test::Nginx->new()->has(qw/http proxy cache age/)->plan(16)
...
like(http_head('/empty.html?head'), qr/^Age: 0.*HIT/ms, 'empty head second');
...
like(http_get('/empty.html'), qr/^Age: 0.*HIT/ms, 'empty get second');

select(undef, undef, undef, 1.1);
like(http_get('/empty.html'), qr/^Age: 1.*HIT/ms, 'empty get third');

select(undef, undef, undef, 2);
unlink $t->testdir() . '/t.html';
like(http_gzip_request('/t.html'),
	qr/HTTP.*^Age: 3.*STALE.*1c\x0d\x0a.{28}\x0d\x0a0\x0d\x0a\x0d\x0a\z/ms,
	'non-empty get stale');

unlink $t->testdir() . '/empty.html';
like(http_gzip_request('/empty.html'),
	qr/HTTP.*^Age: 3.*STALE.*14\x0d\x0a.{20}\x0d\x0a0\x0d\x0a\x0d\x0a\z/ms,
	'empty get stale');
